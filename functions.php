...
function get_content($post, $orderby)
{
...
function get_imgs($posts, $quantit, $id = 2)
{
  ...
  $data['images'] = array_slice(get_field($posts, $id), 0, $quantit);
  $data['amount'] = count(get_field($posts, $id));
  $data['type']   = $posts;
...
function save_dataForm_in_db()
{
  if (isset($_POST['_wpcf7'])) {
    $data = array();
    foreach ($_POST as $key => $value) {
      if ($key != '_wpcf7' && $key != '_wpnonce') {
        $data[$key] = $value;
      }
    }

    include $_SERVER['DOCUMENT_ROOT'] . '/wp-content/themes/theme-maya/inc/connect.php';

    $insert = $conn->prepare("INSERT INTO CREDENCIADOS (data,	sou,	nome,	email,	telefone,	cnpj,	cpf,	empresa,	cidade) VALUES(:datahora, :sou, :nome, :email, :telefone, :cnpj, :cpf, :empresa, :cidade)");
    $insert->execute(array(
      ':datahora' => date('Y-m-d H:i:s'),
      ':sou'       => $data['select-946'],
      ':nome'     => $data['your-name'],
      ':email'    => $data['your-email'],
      ':telefone' => $data['your-tel'],
      ':cnpj'     => $data['your-cnpj'],
      ':cpf'      => $data['your-cpf'],
      ':empresa'  => $data['your-company'],
      ':cidade'   => $data['your-city'],
    ));
  }
}
save_dataForm_in_db();
